...
class VinoRunner(_ModelRunner):
    ...
    def configure(self):
        ...
        ov_model = ov.convert_model(mmodel, example_input=torch.randn((1, 40, 64, 48)))
        self.compiled_model = ov.compile_model(ov_model, "CPU")

        self.output_layer_ir = self.compiled_model.output(0)
        print("vino1 configured")
    ...
    def make_prediction(self, input: np.ndarray):
        ...
        pred = torch.tensor(self.compiled_model(x)[0]).squeeze()
