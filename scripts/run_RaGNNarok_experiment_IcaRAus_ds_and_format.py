...
def run_experiment_cycle(
    ...
    print(f"--- Setting up SageGNNClassifier RaGNNarok Experiment: {config_label} (Base: {actual_config_to_load}) ---")
    config_path = os.path.abspath(os.path.join(os.path.dirname(__file__), f"../configs/RaGNNarok/{actual_config_to_load}.py"))
    ...
    if not skip_benchmark:
        ...
        if torch.cuda.is_available():
            benchmark_model(runner, dataset, input_encoder, "cuda:0", config_label, output_dir, num_runs=num_runs)
        
        # 2. CPU Multi-core Benchmark (System Default)
        benchmark_model(runner, dataset, input_encoder, "cpu", config_label, output_dir, num_runs=num_runs)

        # 3. CPU Single-core Benchmark
        benchmark_model(runner, dataset, input_encoder, "cpu", config_label, output_dir, num_threads=1, num_runs=num_runs)
...
def main():
    ...
    run_experiment_cycle(
        ...
        dataset_path=config.generated_dataset["generated_dataset_path"] if 'config' in locals() else None, # This won't work in main easily without re-loading
